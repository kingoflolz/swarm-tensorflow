...
def _query_tpu_system_metadata(master_address, cluster_def=None,
    ...
    if context.executing_eagerly():
        ...
        logical_devices = [dev for dev in logical_devices if cluster_def.job[0].name in dev.name or "job" not in dev.name]
        ...
        raise NotImplemented("TPU metadata in not eager has not been patched to work with multiple TPUs (from swarm-tensorflow)")
